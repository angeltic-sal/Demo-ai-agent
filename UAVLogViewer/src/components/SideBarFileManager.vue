    ...
    methods: {
        ...
        process: function (file) {
            ...
            reader.onload = async function (e) {
                ...
                // First upload the file to get a log ID
                ...
                try {
                    ...
                    store.logId = result.log_id
                    // Then process the file locally
                    worker.postMessage({
                        action: 'parse',
                        file: data,
                    ...
                    alert('Error uploading file to backend. Chat functionality will not be available.')
                }
            }
        ...
        async handleFileUpload (event) {
            ...
            try {
                ...
                if (response.data && response.data.log_id) {
                    this.$store.commit('setLogId', response.data.log_id)
                    this.$store.commit('setCurrentFile', file.name)
                    this.$store.commit('setFileUploaded', true)
                    this.$store.commit('setFileProcessed', true)
                    this.$store.commit('setFileProcessing', false)
                    this.$store.commit('setFileError', null)
                }
            } catch (error) {
                console.error('Error uploading file:', error)
                this.$store.commit('setFileError', 'Error uploading file')
                this.$store.commit('setFileProcessing', false)
            }
        }
    },
